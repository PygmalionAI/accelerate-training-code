# Monkeypatch LLMs to calculate loss?
...
from aphrodite.modeling.megatron.parallel_state import (
    get_tensor_model_parallel_rank, get_tensor_model_parallel_world_size, gather_from_tensor_model_parallel_region)
...
class GPTNeoXWithLoss(nn.Module):
    def __init__(self, config, is_pretraining) -> None:
        super().__init__()
        ...
        self.embed_out = ColumnParallelLinear(
            ...
            bias=False,
            gather_output=False,
            perform_initialization=is_pretraining
        )
        self.sampler = LossRetriever(config.vocab_size)
    ...
    def forward(
        ...
        logits, loss = self.sampler(self.embed_out.weight, hidden_states, labels)
...
class GPTNeoXAttention(nn.Module):
    ...
    def __init__(self, config: GPTNeoXConfig):
        ...
        self.dense = RowParallelLinear(config.hidden_size,
                                       ...
                                       perform_initialization=config.no_loading_weights)
...
class GPTNeoXWithLoss(nn.Module):
    def __init__(self, config, pretrain: bool = False):
        super().__init__()
        self.config = config
        # Update the config to be able to initialize
        config.no_loading_weights = pretrain
        self.gpt_neox = GPTNeoXModel(config)
        self.embed_out = ColumnParallelLinear(config.hidden_size,
                                              config.vocab_size,
                                              bias=False,
                                              gather_output=False,
                                              perform_initialization=pretrain)
    ...
    def forward(
        ...
        logits, loss = self.sampler(self.embed_out.weight, hidden_states,
                                   input_metadata)
        return loss
