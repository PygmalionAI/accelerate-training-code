# Must modify HF trainer directly to implement MeZO
import math
import os
import time
import typing as t
...
class MeZOOptimizer:
    ...
    def _log_normalize(self, num: float, log_eps: float = 1e-8) -> float:
        '''
...
class MeZOTrainer(Trainer):
    ...
    def _inner_training_loop(
        ...
        if self.use_mezo:
            self.mezo = MeZOOptimizer(self, self.model)
        else:
            self.mezo = None
        ...
        for epoch in range(epochs_trained, num_train_epochs):
            ...
            for step, inputs in enumerate(epoch_iterator):
                ...
                with self.accelerator.accumulate(model):
                    ...
                    if self.use_mezo:
                        tr_loss_step = self.mezo.step(inputs)
                    else:
...
def _linear_probe(model: PreTrainedModel, dataloader: DataLoader, args: TrainingArguments) -> None:
    ...
    with torch.inference_mode():
        ...
        # Whether to use bias
        if model.config.model_type in NO_BIAS_MODEL_TYPES:
            use_bias = False
        else:
